...
pub static MOVE_VM_NATIVE_VERIFY_VDF_PROOF_COUNT: Lazy<IntCounter> = Lazy::new(|| {
    register_int_counter!(
        "diem_move_vm_native_verify_vdf_proof_count", "Cumulative number of verified proofs"
    ).unwrap()
});

pub static MOVE_VM_NATIVE_VERIFY_VDF_PROOF_ERROR_COUNT: Lazy<IntCounter> = Lazy::new(|| {
    register_int_counter!(
        "diem_move_vm_native_verify_vdf_proof_error_count",
        "Cumulative number of errors while verifying proofs"
    ).unwrap()
});